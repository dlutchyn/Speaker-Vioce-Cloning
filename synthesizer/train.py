...
def train(run_id: str, syn_dir: Path, models_dir: Path, save_every: int,  backup_every: int, force_restart: bool,
    ...
    for i, session in enumerate(hparams.tts_schedule):
        ...
        for epoch in range(1, epochs+1):
            running_loss_m1 = 0.0
            running_loss_m2 = 0.0
            running_loss_binary_cross_entropy = 0.0
            running_loss = 0.0
            for i, (texts, mels, embeds, idx) in enumerate(data_loader, 1):
                ...
                running_loss_m1 += m1_loss.item()
                running_loss_m2 += m2_loss.item()
                running_loss_binary_cross_entropy += stop_loss.item()
                ...
                if step >= max_step:
                    break
            logs = {
                'general_loss': running_loss / epochs,
                'm1_loss': running_loss_m1 / epochs,
                'm2_loss': running_loss_m2 / epochs,
                'binary_cross_entropy': running_loss_binary_cross_entropy / epochs
            }
            if use_wandb:
                wandb.log(logs)
